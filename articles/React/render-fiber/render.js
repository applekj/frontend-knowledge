...
const workLoop = deadLine => {
    ...
    if (!nextFiberReconcileWork && wipRoot) { //如果reconcile任务清空了，进入到commit阶段
        commitRoot()
    }
...
const performNextWork = fiber => {
    let children = null
    if (Component.isPrototypeOf(fiber.type)) { //如果是class组件
        wipFunFiber = fiber
        hookIndex = 0
        wipFunFiber.hooks = []
        children = [transfer(fiber.type)(fiber.props)]
    } else if (fiber && fiber.type && fiber.type instanceof Function) { //如果fiber是函数组件
        wipFunFiber = fiber
        hookIndex = 0
        wipFunFiber.hooks = []
